...
def add_cost_task():
    schedule_id = int(session.cost_schedule_selector.split("/", 1)[1]) if session.cost_schedule_selector else None
    if not schedule_id:
        st.warning("Please select a cost schedule first.")
        return
    schedule = session.ifc_file.by_id(schedule_id)
    if not schedule:
        st.warning("The selected schedule does not exist.")
        return
    ...
    task_cost = st.number_input("Task Cost", min_value=0.0, step=0.01)
    task_date = st.date_input("Task Date")
    ifchelper.create_cost_task(session.ifc_file, schedule, task_name, task_description, task_cost, task_date)
    load_cost_schedules()
...
def delete_cost_task():
    ...
    ifchelper.delete_cost_task(session.ifc_file, schedule, task_id)
    load_cost_schedules()
